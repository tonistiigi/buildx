...
func Parse(b hcl.Body, opt Opt, val interface{}) hcl.Diagnostics {
	...
	schema, partial := gohcl.ImpliedBodySchema(val)
	...
	log.Printf("schem %+v %v", schema, partial)
	log.Printf("values: %v", p.ectx.Variables)
	...
	for _, b := range content.Blocks {
		...
		log.Printf("lbl %q %v", b.Type, b.Labels)
	}

	log.Printf("content %+v %v", content, remain)
